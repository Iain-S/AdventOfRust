...
fn can_form_two(test_val: u64, mut nums: VecDeque<u64>) -> (bool, String) {
    let first = nums.pop_back().unwrap();
    // Our stopping condition.
    if nums.len() == 0 {
        if first == test_val {
    ...
    if test_val >= first {
        let add = can_form_two(test_val - first, nums.clone());
        if add.0 {
            return (true, add.1 + "+");
        }
    }
    ...
    if test_val % first == 0 {
        let mul = can_form_two(test_val / first, nums.clone());
        if mul.0 {
            return (true, mul.1 + "*");
        }
    }

    // We know we have at least one more number.
    // let last = nums[nums.len() - 1];
    // let combined = last.to_string() + first.to_string().as_str();
    // if last.to_string() + first.to_string().as_str() == test_val.to_string() {
    // return (true, "||".to_owned());
    // }
    let first_string = first.to_string();
    let first_string_len = first_string.len();
    let test_val_string = test_val.to_string();
    let test_val_string_len = test_val_string.len();
    if test_val_string_len > first_string_len && test_val_string.ends_with(first_string.as_str()) {
        let index = test_val_string.len() - first_string.len();
        let concatt = can_form_two(
            test_val_string[..index].parse::<u64>().unwrap(),
            nums.clone(),
        );
        if concatt.0 {
            return (true, concatt.1 + "|");
        }
    }
...
fn part_two(s: &str) -> u64 {
    ...
    for (test_val, nums) in myvec.iter() {
        let test_val_val = *test_val;
        let debug_nums = nums.clone();
        let canform = can_form_two(*test_val, nums.clone());
        if canform.0 {
            let mut clone = nums.clone();
            let mut subtotal = clone.pop_front().unwrap();
            for c in canform.1.chars() {
                if c == '+' {
                    ...
                    subtotal = (subtotal.to_string()
                        + clone.pop_front().unwrap().to_string().as_str())
                    .parse::<u64>()
                    .unwrap();
                } else {
                    panic!("Unexpected character: {}", c);
...
mod tests {
    ...
    #[test]
    fn test_can_form_two() {
        let result = can_form_two(156, vec![15, 6].into());
        assert_eq!(result.0, true);

        let result = can_form_two(312, vec![15, 6, 2].into());
        assert_eq!(result.0, true);
    }

    #[test]
    fn test_part_two() {
        let result = part_two(&example_input("07"));
        assert_eq!(result, 11_387);
    }
}
